...
# Entrenar cada modelo y recopilar datos
for model_type, model_name in model_types.items():
    print(f"\n== Entrenando: {model_name} ==")
    trainer = MnistTrainer(model_type=model_type, num_epochs=30, learning_rate=0.001, batch_size=64)
    ...
    for epoch in range(1, trainer.num_epochs + 1):
        ...
        val_loss, val_acc = trainer.evaluate_on_loader(trainer.val_loader, loader_name="Validación")

        val_accuracies.append(val_acc)
        val_losses.append(val_loss)

        if model_type == MNISTModelType.DECISION_TREE:
            break  # Solo una pasada para árboles
